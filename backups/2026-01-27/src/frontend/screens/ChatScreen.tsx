...
import { Bot, Mic, ArrowLeft, Bookmark, Check } from 'lucide-react';
...
const ChatContent = () => {
  ...
  useEffect(() => {
    ...
    if (lastMsg && lastMsg.isDecisionPoint && lastMsg.evidenceCards) {
      ...
      // 3초 후 이동 (사용자가 메시지를 보고 '저장' 버튼을 누를 여유 제공)
  ...
  const handleSaveCourse = async (msg: Message) => {
    if (!msg.evidenceCards) return;
    setSavingId(msg.id);
    ...
    const courseData = {
      owner_id: userId,
      title: "추천 여행 코스", // 백엔드에서 AI가 업데이트 예정
      places: msg.evidenceCards.map(c => ({
        id: c.placeId,
        name: c.name,
        lat: c.lat,
        lng: c.lng,
        vibe_tags: c.keywords || []
      }))
    };

    const success = await aiService.saveCourse(courseData);
    if (success) {
      setSavingId('saved_' + msg.id);
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-10 hide-scrollbar pb-60">
        {messages.map((m) => (
          <div key={m.id} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} animate-fade-in`}>
            ...
            {/* 코스 결과가 나왔을 때 (isDecisionPoint) */}
            {m.isDecisionPoint && (
              <div className="flex flex-col gap-4 w-[85%]">
                <div className="p-6 bg-blue-50 text-[#0066FF] rounded-[2rem] font-bold text-[15px] border border-blue-100 shadow-sm">
                  {m.text || "완벽한 여행 코스를 생성했습니다!"}
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleSaveCourse(m)}
                    className={`flex-1 flex items-center justify-center gap-2 py-4 rounded-2xl font-bold text-sm transition-all ${savingId === 'saved_' + m.id
                        ? 'bg-green-500 text-white'
                        : 'bg-white border border-gray-100 text-gray-700 shadow-sm hover:shadow-md'
                      }`}
                  >
                    {savingId === 'saved_' + m.id ? <Check size={18} /> : <Bookmark size={18} className="text-[#0066FF]" />}
                    {savingId === 'saved_' + m.id ? '저장됨' : 'AI ARCHIVE에 저장'}
                  </button>
                  <button
                    onClick={() => router.push('/map')}
                    className="flex-1 bg-[#0066FF] text-white py-4 rounded-2xl font-bold text-sm shadow-blue-100 shadow-lg active:scale-95 transition-all"
                  >
                    지도로 확인하기
                  </button>
                </div>
