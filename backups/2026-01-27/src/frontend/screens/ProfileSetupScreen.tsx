    ...
    const handleComplete = async () => {
        if (gender && age) {
            try {
                ...
                // 1. 회원 여부에 따라 API 호출 분기
                const isMember = accessToken && tempUserId && !tempUserId.includes('-');

                let response;
                if (isMember) {
                    // 구글 ID 형태(보통 숫자로만 이루어짐)이거나 토큰이 있으면 회원으로 보아 PUT 호출
                    // UUID 형태인 경우 게스트로 판단 (uuid.v4()는 하이픈 포함)
                    response = await fetch(`${apiUrl}/api/user/profile?userId=${tempUserId}`, {
                        method: 'PUT',
                        headers: {
                    ...
                    // 게스트용 POST 호출
                    response = await fetch(`${apiUrl}/api/user/onboard`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                ...
                if (response.ok) {
                    ...
                    // 2. 받은 임시 ID(userId)를 브라우저에 저장 (게스트일 경우 신규 ID)
                    if (data.userId) {
