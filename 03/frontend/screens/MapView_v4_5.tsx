...
import { motion, useAnimation, PanInfo, Reorder, AnimatePresence } from 'framer-motion';
...
export const MapView = () => {
  ...
  // [v4.3] Journey Completion Modal
  const [showCompletionModal, setShowCompletionModal] = useState(false);

  // [v4.4] Auto-Resume State
  const [showResumeModal, setShowResumeModal] = useState(false);
  const [resumeState, setResumeState] = useState<{ index: number, total: number } | null>(null);
  ...
  // Helper: Ensure IDs and data structure for spots
  ...
  useEffect(() => {
    ...
    if (autoGen === 'true') {
      ...
      const init = async () => {
        // [v4.2] Hydration Priority: Check for Auto-Resume first
        const storedState = localStorage.getItem('current_course_state');
        if (storedState) {
          try {
            const parsed = JSON.parse(storedState);
            if (parsed.current_step_index > 0 && parsed.current_step_index < parsed.total_steps) {
              setResumeState({ index: parsed.current_step_index, total: parsed.total_steps });
              setShowResumeModal(true);
              console.log("📍 [MapView] Hydration: Auto-Resume triggered", parsed.current_step_index);
            }
          } catch (e) { console.error("Resume check failed", e); }
        }

        try {
          // [New v4.2] Priority Loading: mode=course_navigator인 경우 current_course를 최우선으로 로드
          const mode = searchParams.get('mode');
          if (mode === 'course_navigator') {
            const stored = localStorage.getItem('current_course');
            if (stored) {
              const parsed = JSON.parse(stored);
              setSpots(ensureIds(parsed));
              setPickedSpots(parsed);
              setViewMode('course');
  ...
  const handleResume = () => {
    if (resumeState) {
      setActiveStep(resumeState.index);
      setSheetOpen(true);
    }
  ...
  useEffect(() => {
    const checkInvitation = async () => {
      ...
      // [DEMO CHANGE] Use sessionStorage to allow recurring exposure on page refresh (F5)
      const hasSeenInSession = sessionStorage.getItem(`invitation_seen_${userId}`);
      if (hasSeenInSession === 'true') {
        console.log("ℹ️ [Invitation] Already seen in this session. Skipping...");
        return;
      }

      try {
        // [DEMO CHANGE] We can still fetch profile, but we bypass the permanent DB check if needed
        // For now, let's keep it clean: if not seen in session, show it.
        console.log("💌 [Invitation] Fetching hard-coded courses for session...");

        // [BUG FIX] Handle potential double /api
        // NEXT_PUBLIC_API_URL="http://localhost:8000/api"
        const API_BASE = (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api').replace(/\/$/, '');

        // If API_BASE already ends with /api, don't add it again
        const generateUrl = API_BASE.endsWith('/api')
          ? `${API_BASE}/invitation/generate/${userId}`
          : `${API_BASE}/api/invitation/generate/${userId}`;

        const seenUrl = API_BASE.endsWith('/api')
          ? `${API_BASE}/invitation/seen/${userId}`
          : `${API_BASE}/api/invitation/seen/${userId}`;

        // 2. Generate Courses (Now Hard-coded in Backend)
        ...
        if (res.ok) {
          ...
          if (courses && courses.length > 0) {
            ...
            // 3. Mark as seen in SESSION to prevent UI spam within same tab
            sessionStorage.setItem(`invitation_seen_${userId}`, 'true');

            // Still update DB for consistency
            await fetch(seenUrl, { method: 'PATCH' });
          }
        }
      } catch (e) {
      ...
      displayData.forEach((spot, index) => {
        ...
        const onMarkerClick = async () => {
          if (viewMode === 'course') {
            ...
            // Progress sync is handled by the dedicated useEffect
          }
  ...
  // [v4.2] Progress Sync Effect: Ensures all step changes are saved for Auto-Resume
  useEffect(() => {
    if (viewMode === 'course' && spots.length > 0) {
      const userId = localStorage.getItem('access_token') || localStorage.getItem('temp_user_id') || 'guest';

      // Update local snapshot for auto-resume
      localStorage.setItem('current_course_state', JSON.stringify({
        current_step_index: activeStep,
        total_steps: spots.length,
        updatedAt: new Date().toISOString()
      }));

      // Sync to backend if not guest
      if (userId !== 'guest' && activeStep > 0) {
        fetch(`http://localhost:8000/api/session/step/${userId}?current_step_index=${activeStep}`, {
          method: 'PATCH'
        }).catch(e => console.error("Step sync failed", e));
      }
    }
  }, [activeStep, spots.length, viewMode]);
  ...
  return (
    <div className="h-screen bg-gray-50 relative overflow-hidden font-['Inter']">
              ...
              isCourseDetailExpanded ? (
                ...
                <div className="animate-fade-in space-y-6 pt-2 h-full flex flex-col overflow-y-auto pb-48 custom-scrollbar">
                  ...
                  <div className="flex gap-3 shrink-0">
                    ...
                    {activeStep === spots.length - 1 ? (
                      <button
                        onClick={() => setShowCompletionModal(true)}
                        className="flex-1 py-4 bg-[#FF6B00] text-white rounded-2xl font-black text-sm shadow-lg shadow-orange-100 animate-bounce-slow active:scale-95 transition-all"
                      >
                        여정 완료하기 🥳
                      </button>
                    ) : (
                      <button onClick={nextStep} className="flex-1 py-4 bg-gray-100 text-gray-800 rounded-2xl font-black text-sm active:scale-95 transition-all">다음 장소</button>
                    )}
                  </div>
      ...
      <InvitationPopup
        ...
        invitationData={{
          id: 'discovery-entry',
          title: "7가지 특별한 테마가 준비되었습니다",
          description: "여행자님의 취향에 딱 맞는 코스를 발견하고 직접 커스터마이징해 보세요.",
          imageUrl: "https://images.unsplash.com/photo-1582234373453-909778434800?w=800"
        }}
      />

      {/* [v4.4] Auto-Resume Modal */}
      <AnimatePresence>
        {showResumeModal && (
          <div className="fixed inset-0 z-[1100] flex items-center justify-center px-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-sm bg-white rounded-[2.5rem] overflow-hidden shadow-2xl p-8 flex flex-col items-center text-center"
            >
              <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center text-4xl mb-6">📍</div>
              <h2 className="text-2xl font-black text-gray-900 mb-2 leading-tight">여정을 이어갈까요?</h2>
              <p className="text-sm text-gray-500 font-medium mb-8">
                방금 전까지 <strong>광주 여행({resumeState?.index || 0 + 1} / {resumeState?.total})</strong>을<br />
                즐기고 계셨네요. 마지막 지점에서 이어갈까요?
              </p>

              <div className="w-full flex flex-col gap-3">
                <button
                  onClick={handleResume}
                  className="w-full py-4 bg-[#0066FF] text-white font-black rounded-2xl shadow-lg shadow-blue-100 active:scale-95 transition-all"
                >
                  마지막 지점에서 시작하기
                </button>
                <button
                  onClick={() => {
                    setShowResumeModal(false);
                    localStorage.removeItem('current_course_state');
                  }}
                  className="w-full py-4 bg-gray-50 text-gray-400 font-bold rounded-2xl active:scale-95 transition-all"
                >
                  처음부터 보기
                </button>
              </div>
      ...
      {/* [v4.3] Journey Completion Celebration Modal */}
      <AnimatePresence>
        {showCompletionModal && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center px-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-sm bg-white rounded-[2.5rem] overflow-hidden shadow-2xl p-8 flex flex-col items-center text-center"
            >
              <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center text-4xl mb-6">🥳</div>
              <h2 className="text-2xl font-black text-gray-900 mb-2 leading-tight">오늘의 광주 여행은<br />어떠셨나요?</h2>
              <p className="text-sm text-gray-500 font-medium mb-8">여행자님의 소중한 경험을<br />'테이스팅 노트'로 남겨주세요.</p>

              <div className="w-full flex flex-col gap-3">
                <button
                  onClick={() => {
                    if (checkIsGuest()) {
                      setModalFeature("테이스팅 노트 작성");
                      setShowLoginModal(true);
                    } else {
                      // 테이스팅 노트 작성 페이지로 이동
                      // 이동 전 현재 코스를 확실히 싱크 (혹시 모를 유실 방지)
                      localStorage.setItem('current_course', JSON.stringify(spots));
                      router.push('/tasting-note');
                    }
                  }}
                  className="w-full py-4 bg-[#FF6B00] text-white font-black rounded-2xl shadow-lg shadow-orange-100 active:scale-95 transition-all"
                >
                  테이스팅 노트 작성하기
                </button>
                <button
