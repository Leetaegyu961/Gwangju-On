...
export default function TimelineScreen() {
    ...
    useEffect(() => {
        ...
        // proV4 전용: 찜 목록에서 바로 넘어온 경우 처리
        if (searchParams.get('mode') === 'create_memory') {
            const loadAndOpen = async () => {
                const updatedStored = localStorage.getItem('current_course');
                const spots = updatedStored ? JSON.parse(updatedStored) : [];
                if (spots.length > 0) {
                    const currentAlbum: Album = {
                        id: 'current',
                        title: '광주에서 보낸 오후',
    ...
    const initMiniMap = () => {
        ...
        if (currentCourse.length > 0) {
            centerLat = parseFloat(currentCourse[0].lat || 35.1595);
            centerLng = parseFloat(currentCourse[0].lng || 126.8526);
        }
        ...
        // 4. 지도 영역 맞춤
        setTimeout(() => {
            if (miniMapInstance.current && !bounds.isEmpty()) {
    ...
    useEffect(() => {
        ...
        if (currentSlide <= course.length || isDownloading || !isCardModalOpen) return;
    ...
    }, [currentSlide, isDownloading, isCardModalOpen, course.length]); // isDownloading이 false가 되면 다시 실행됨
        ...
        if (isDownloading) {
            ...
            if (validSpots.length >= 2) {
                ...
                svgPoints = validSpots.map(s => {
                    ...
                    return { x: x * 100, y: y * 100, name: s.place_name || s.name };
                });
            } else {
                svgPoints = course.map((s, i) => ({ x: 20 + i * 30, y: 80 - i * 25, name: s.place_name || s.name })).slice(0, 3);
            }
                                ...
                                <div className="w-full aspect-[16/9] rounded-xl overflow-hidden shadow-sm relative">
                                    ...
                                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-2 text-white pt-6">
                                        <div className="flex items-center gap-1 mb-0.5">
                                            ...
                                            <span className="text-[9px] font-bold truncate">{spot.place_name || spot.name}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    {course.map((spot, index) => (
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} key={index} className="flex gap-5">
                            ...
                            <div className="flex-1 bg-white p-5 rounded-2xl shadow-sm border border-gray-100/50">
                                <div className="mb-4">
                                    <h3 className="text-lg font-black text-gray-800">{spot.place_name || spot.name}</h3>
                                                    ...
                                                    >
                                                        <div className="flex justify-between items-end border-b-2 border-dashed border-gray-200 pb-4 mb-6">
                                                            <div>
                                                                ...
                                                                <h2 className="text-2xl font-black text-gray-800 leading-tight">{spot.place_name || spot.name}</h2>
                                                            </div>
